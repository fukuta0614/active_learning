"""
 11-layer ConvNet (6.1% and 26.7% on CIFAR-10 and CIFAR-100)
 python main.py --dataset [dataset] --netType convnet --data [path/to/data/directory]
...
def main():
    ...
    for i in range(1, opt.nTrials + 1):
        print('+-- Trial {}/{} --+'.format(i, opt.nTrials))
        train(opt, i)
...
def train(opt, trial):
    ...
    if opt.save != 'None':
        ...
        chainer.serializers.save_npz(
            os.path.join(opt.save, 'optimizer_trial{}.npz'.format(trial)), optimizer)
