...
from train_utils import make_optimizer, progress_report, random_crop, evaluate
...
from models.dirty_originals import ResNet50, GoogLeNet, VGG16
from PIA.camelyon import CamelyonDataset, dataset_path, extracted_16_dir, extracted_17_dir
import debugger
...
def main():
    ...
    # perm = np.random.permutation(len(train_data))[:int(len(train_data) * args.data_size)]
    # train_data = train_data[perm]
    preprocess_type = args.arch if not args.hed else 'hed'
    train = CamelyonDataset(train_data,
                            original_size=image_size, crop_size=crop_size, aug=False, color_aug=False,
                            num_class=num_class, from_tif=False, preprocess_type=preprocess_type)
    ...
    for diag in test_data:
        ...
        test = CamelyonDataset(test_data[diag],
                               original_size=image_size, crop_size=crop_size, aug=False, color_aug=False,
                               num_class=num_class, from_tif=False, preprocess_type=preprocess_type)
        diag_iter[diag] = iterators.MultiprocessIterator(test, args.batch_size, repeat=False, shuffle=False)
    ...
    if feat_type == 'cbp':
        ...
        for diag in diag_iter:
            ...
            count = 0
            for batch in tqdm(it, total=len(it.dataset) // args.batch_size):
                x, t = convert.concat_examples(batch, device)
                model(x, t)
                feat[count: count + len(batch)] = cuda.to_cpu(model.feat.data)
                count += len(batch)
            np.save('test_' + diag + '_cbp512_feat.npy', feat)
